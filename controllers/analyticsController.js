...
async function getTopOrderedItems(req, res) {
  try {
    const items = await analyticsModel.getTop3ForPatron();
    res.status(200).json(items);
  } catch (err) {
    console.error("Controller error in getTopOrderedItems:", err);
    res.status(500).json({ message: "Failed to retrieve top ordered items." });
  }
}
